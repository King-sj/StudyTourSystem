...
使用KMP算法进行查询，然后将输出的返回值进行特定的排序

首先实现KMP算法进行查找，以下是KMP查询的主要算法

主要的搜索函数，接收两个字符串参数：模式pat和文本txt。它返回模式在文本中的所有出现位置的列表。
...
辅助函数，用于预处理模式字符串并填充LPS数组。LPS数组包含了在不匹配时应该跳过的字符数，从而避免不必要的比较。
```python
'''
@param pat 模式
@param M 模式长度
...
在类实例的文本属性中搜索关键字函数接受一个关键字和一系列数据类实例。它在每个实例的属性中搜索该关键字，如果属性是字符串、列表或集合，并且元素类型为字符串，则使用KMP算法进行搜索。搜索结果以实例为键，属性名和关键字出现次数为值的字典形式返回。
