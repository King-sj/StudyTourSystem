...
def get_crowd(road: Road, start_degree, destination_degree, average_degree):
  ...
  if current_time.tm_hour >= 0 and current_time.tm_hour < 7:
    time_coefficient = 0.5
  elif current_time.tm_hour >= 7 and current_time.tm_hour < 10:
    time_coefficient = 2
  elif current_time.tm_hour >= 10 and current_time.tm_hour < 17:
    time_coefficient = 1.5
  elif current_time.tm_hour >= 17 and current_time.tm_hour < 20:
    time_coefficient = 2
  else:
    time_coefficient = 0.5
  busy_coefficient: float = (
      start_degree + destination_degree) / 2.0 / average_degree
  ...
  for value in area.road_group.values():
    for r in value:
      if r == road:
        check = True
  if not check:
    raise AttributeError('Road not in area')
